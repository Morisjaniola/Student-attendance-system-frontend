      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] text-left">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {grouped.map((group) => <GroupRows key={group.key} group={group} showHeading={Boolean(groupMode)} />)}
          </tbody>
        </table>
      </div>
    </section>
...
function GroupRows({ group, showHeading }: { group: { key: string; label: string; records: AttendanceRecord[] }; showHeading: boolean }) {
  return <>
    ...
    {group.records.map((record) => <tr key={record.id} className="text-xs text-slate-600 transition hover:bg-slate-50/80 dark:text-slate-300 dark:hover:bg-slate-800/40">
      ...
      <td className="px-3 py-3.5 font-mono text-[11px] text-slate-400">{record.student.studentId}</td><td className="px-3 py-3.5 font-semibold">{record.student.courseCode}</td><td className="px-3 py-3.5">{record.student.section}</td><td className="px-3 py-3.5 text-[11px] text-slate-400">{record.dateLabel}</td><td className="px-3 py-3.5 font-mono text-[11px] text-slate-500">{record.time}</td><td className="px-5 py-3.5"><AttendanceStatusBadge status={record.status} /></td>
    </tr>)}
  </>
}
