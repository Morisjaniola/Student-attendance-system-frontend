...
interface CreateUserModalProps {
  open: boolean
  loading: boolean
  onSave: (values: UserFormValues) => Promise<unknown>
  onCancel: () => void
}

export function CreateUserModal({ open, loading, onSave, onCancel }: CreateUserModalProps) {
  const [serverError, setServerError] = useState<string | null>(null)
  const form = useForm<UserFormValues>({
    resolver: zodResolver(createSchema),
    // New accounts default to Active (requirement 10.2).
    defaultValues: { name: '', username: '', email: '', role: 'Staff', status: 'Active' },
  })

  useEffect(() => {
    if (open) {
      form.reset({ name: '', username: '', email: '', role: 'Staff', status: 'Active' })
      setServerError(null)
    }
  }, [form, open])
  ...
  const submit = async (values: UserFormValues) => {
    ...
    try {
      ...
      setServerError(error instanceof Error ? error.message : 'Unable to create the user account. Please try again.')
    }
  }

  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-0 backdrop-blur-sm sm:p-5">
      <section role="dialog" aria-modal="true" aria-labelledby="create-user-title" className="mx-auto min-h-full w-full max-w-lg bg-white shadow-2xl dark:bg-slate-900 sm:min-h-0 sm:rounded-2xl">
        <header className="flex items-start justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800 sm:px-6">
          <div>
            <h2 id="create-user-title" className="flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white"><UserPlus size={19} className="text-blue-600" />Create user account</h2>
            <p className="mt-1 text-xs text-slate-400">Add a new administrator, faculty, or staff account.</p>
          </div>
          <button onClick={onCancel} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Close create user dialog">
            <X size={19} />
          </button>
        </header>
