...
export function AttendanceSettingsCard({ value, saving, onChange, onSave }: AttendanceSettingsCardProps) {
  ...
  return (
    <SettingsCard title="Attendance Settings" description="Set the daily attendance window and rules for late arrivals." icon={<CalendarClock size={19} />} saving={saving} onSave={onSave}>
      ...
      <div className="border-t border-slate-100 dark:border-slate-800"><SettingToggle id="allow-late-attendance" label="Allow late attendance" description="Record attendance after the late threshold and mark the entry as late." checked={value.allowLateAttendance} onChange={(checked) => update('allowLateAttendance', checked)} /></div>
