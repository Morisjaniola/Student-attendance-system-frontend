  ...
  return (
    <SettingsCard title="Notification Settings" description="Control attendance alerts while remaining compatible with the existing Notifications module." icon={<BellRing size={19} />} saving={saving} onSave={onSave}>
      <div className="divide-y divide-slate-100 dark:divide-slate-800">
        <SettingToggle id="system-notifications" label="Enable notifications" description="Show attendance-related notifications in the application." checked={value.notificationsEnabled} onChange={(checked) => update('notificationsEnabled', checked)} />
        <SettingToggle id="late-student-alerts" label="Late student alerts" description="Create an alert when a student is recorded after the late threshold." checked={value.lateStudentAlerts} disabled={!value.notificationsEnabled} onChange={(checked) => update('lateStudentAlerts', checked)} />
        <SettingToggle id="attendance-confirmations" label="Attendance confirmation notifications" description="Show a confirmation notification after an attendance entry is recorded." checked={value.attendanceConfirmationNotifications} disabled={!value.notificationsEnabled} onChange={(checked) => update('attendanceConfirmationNotifications', checked)} />
      </div>
