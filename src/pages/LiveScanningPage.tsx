...
/** Dedicated workstation for recording attendance with QR Code or RFID. */
export function LiveScanningPage() {
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-3xl">Live Scanning</h1>
      ...
      <AttendanceScanner activeMethod={activeMethod} onMethodChange={setActiveMethod} onScan={handleScan} busy={busy} result={result} />
      <AttendanceConfirmation record={result?.outcome === 'success' ? result.record : null} open={confirmOpen} onClose={() => setConfirmOpen(false)} />
      <DuplicateAttendanceDialog previous={result?.outcome === 'duplicate' ? result.previous : null} open={duplicateOpen} onClose={() => setDuplicateOpen(false)} />
    </div>
  )
}
