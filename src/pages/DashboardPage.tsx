...
export function DashboardPage() {
  ...
  return <div className="space-y-6"><WelcomeCard /><section className="grid grid-cols-1 gap-4 min-[500px]:grid-cols-2 xl:grid-cols-5">{data.attendanceStats.map((metric, index) => <StatCard key={metric.id} metric={metric} index={index} />)}</section><section className="grid gap-5 xl:grid-cols-12"><div className="xl:col-span-8"><AttendanceChart data={data.trend} isLoading={isPending} /></div><div className="grid gap-5 sm:grid-cols-2 xl:col-span-4 xl:grid-cols-1"><PieChartCard data={data.distribution} /><ClockWidget /></div></section><section className="grid gap-5 lg:grid-cols-2 xl:grid-cols-12"><div className="xl:col-span-4"><StudentOverviewCard {...data.studentStats} /></div><div className="xl:col-span-4"><QRCard scan={data.qr} /></div><div className="xl:col-span-4"><RFIDCard scan={data.rfid} lastScan={data.rfid.lastScan} /></div></section><section className="grid gap-5 xl:grid-cols-12"><div className="xl:col-span-7"><BarChartCard data={data.courseAttendance} /></div><div className="xl:col-span-5"><QuickActions onAction={(action) => navigate(actionRoutes[action] ?? '/dashboard')} /></div></section><ActivityTable data={data.activities} onView={viewStudent} /><UpcomingEvents events={data.events} onView={viewEvent} /><section className="grid gap-5 lg:grid-cols-2"><AnnouncementPanel announcements={data.announcements} /><NotificationPanel notifications={data.notifications} /></section>{notice && <div className="fixed bottom-5 right-5 z-50 flex max-w-sm items-center gap-3 rounded-xl bg-slate-900 px-4 py-3 text-sm text-white shadow-2xl dark:bg-white dark:text-slate-900" role="status"><span className="flex-1">{notice}</span><button onClick={() => setNotice(null)} className="rounded p-1 hover:bg-white/15 dark:hover:bg-slate-100" aria-label="Dismiss message"><X size={16} /></button></div>}{selectedEvent && <EventDetailModal event={selectedEvent} onClose={() => setSelectedEvent(null)} />}</div>
}
